...
#define ARCHIVO "../Season_Sp_Region_4_Results/r4E1gs"
/***********************************************/
...
/*     fsal1 = fopen("../Season_Sp_Region_4_Results/r4E1gs_x.txt","w"); */
/*     fsal2 = fopen("../Season_Sp_Region_4_Results/r4E1gs_e.txt","w"); */
/*     fsal3 = fopen("../Season_Sp_Region_4_Results/r4E1gs_p.txt","w"); */
...
/************************************************/
  ...
  FILE* data = fopen("../Season_Sp_Region_4_Data/r4E1_datos.txt","r");
      if (data==NULL)
      {
      ...
      for(i=0;i<nSitios;i++){
      fscanf(data,"%d %d %d",&i,&D[i],&X[i]);
      }
  fclose(data);

  FILE* neighbor = fopen("../../../Neighborhood _Structure/r4_vecinos.txt","r");
      if (neighbor==NULL)
      {
  	  ...
  	  for(i=0;i<nSitios;i++){
  	   fscanf(neighbor,"%d",&V[i]);
  		for(j=0;j<V[i];j++){
  		  fscanf(neighbor,"%7d",&N[i][j]);
  		}
      }
  fclose(neighbor);
